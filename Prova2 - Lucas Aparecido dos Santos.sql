-----------------------------------------------------------------------------------
-- Prova P2 / Banco de Dados 2 (CJOBDD2)
-- Nome do aluno: Lucas Aparecido dos Santos (CJ3032116)
-- Data da prova: 12/11/2025
-----------------------------------------------------------------------------------
...
SELECT
    NumeroPedido AS Pedido,
    COUNT(DISTINCT CodigoProduto) AS QtdProdutosDiferentes,
...
SELECT
    ...
    NomeVendedor AS Nome,
    Salario,
    (SELECT AVG(Salario) FROM VENDEDORES) AS MediaSalarial
FROM
    VENDEDORES
WHERE
...
SELECT
    ...
    PR.DescricaoProduto AS Produto,
    PR.ValorUnitario AS ValorUnitario,
    IP.Quantidade AS QuantidadeComprada,
    (IP.Quantidade * PR.ValorUnitario) AS ValorTotalItem,
    PE.PrazoEntrega AS PrazoEntrega,
    V.NomeVendedor AS NomeVendedor
FROM
    CLIENTES AS C
INNER JOIN
    PEDIDOS AS PE ON C.CodigoCliente = PE.CodigoCliente
INNER JOIN
    VENDEDORES AS V ON PE.CodigoVendedor = V.CodigoVendedor
INNER JOIN
    ITEM_DE_PEDIDO AS IP ON PE.NumeroPedido = IP.NumeroPedido
INNER JOIN
    PRODUTOS AS PR ON IP.CodigoProduto = PR.CodigoProduto
ORDER BY
    NomeCliente,
    Produto,
    QuantidadeComprada,
